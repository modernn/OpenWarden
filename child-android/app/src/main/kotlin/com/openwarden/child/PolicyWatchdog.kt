...
/**
 * The self-healing policy watchdog. A single [reassert] re-applies the full local policy so the
 * child stays locked down even if a restriction is cleared, a bundle changes, or connectivity
 * flips. [PolicyService] drives it from three triggers (ADR-021): boot, connectivity change, and
 * a periodic timer.
 *
    ...
    companion object {
        ...
        fun forContext(context: Context): PolicyWatchdog {
            ...
            return PolicyWatchdog(
                ...
                reassertAllowlist = {
                    store.loadActive()?.let { enforcer.applyAllowlist(it.policy.allowlist.toSet()) }
                },
