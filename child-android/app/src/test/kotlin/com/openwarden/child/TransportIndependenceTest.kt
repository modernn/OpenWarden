...
class TransportIndependenceTest {
    ...
    private fun cmd(type: String = SignedCommand.TYPE_LOCK, issuedAt: Long = now) =
    ...
    fun `a tampered command is rejected even imagining a fully trusted socket`() {
        ...
        val tampered = CommandTestSigner.sign(cmd(type = SignedCommand.TYPE_UNLOCK), kp)
        ...
        assertFalse(
            accepts(
                CommandAdmission.decide(
                    tampered, SignedCommand.TYPE_LOCK, myId, kp.pubRaw, commandFloor = null, nowMs = now,
                ),
            ),
        )
    }
    ...
    fun `a genuine command is admitted independent of transport`() {
        ...
        assertTrue(
            accepts(
                CommandAdmission.decide(
                    genuine, SignedCommand.TYPE_LOCK, myId, kp.pubRaw, commandFloor = null, nowMs = now,
                ),
            ),
        )
    }
