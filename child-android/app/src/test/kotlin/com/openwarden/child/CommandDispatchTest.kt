...
class CommandDispatchTest {
    ...
    private fun cmd(type: String = SignedCommand.TYPE_LOCK) =
        SignedCommand(v = 1, type = type, child_device_id = myId, issued_at = 1L)

    @Test
    fun `null command (unparseable body) maps to 400 MALFORMED`() {
        val resp = CommandDispatch.dispatch(null, SignedCommand.TYPE_LOCK, pinned) { _, _, _ ->
    ...
    fun `accepted lock maps to 200 locked`() {
        val resp = CommandDispatch.dispatch(cmd(SignedCommand.TYPE_LOCK), SignedCommand.TYPE_LOCK, pinned) { _, _, _ ->
    ...
    fun `accepted unlock maps to 200 unlocked`() {
        val resp = CommandDispatch.dispatch(cmd(SignedCommand.TYPE_UNLOCK), SignedCommand.TYPE_UNLOCK, pinned) { _, _, _ ->
    ...
    fun `rejected command maps to 400 REJECTED with the reason`() {
        val resp = CommandDispatch.dispatch(cmd(), SignedCommand.TYPE_LOCK, pinned) { _, _, _ ->
