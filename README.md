...
- **Setup (Android):** **no computer needed** — factory-reset the kid's phone, scan a QR from the parent app, and the child app installs from the Play Store and locks in as Device Owner ([ADR-027](docs/adr/027-provisioning-distribution-model.md)). The factory reset + QR scan is what makes it un-bypassable; there is **no** "install over your existing phone" mode — that can't deliver real protection. (A USB/ADB path exists for power users.)
