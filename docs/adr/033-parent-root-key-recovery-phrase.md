...
**D7 — Confirm-back gate is fail-closed and testable.** `commonMain` `ConfirmGate` state machine: after display, the parent re-types 6 randomly-chosen word indices; the gate stays `Unconfirmed` until **all six** match, then emits `Confirmed`. **Until `Confirmed`, the derived key is not persisted and `RootKeyProvider` stays not-provisioned** — a parent who scrolled past without reading cannot proceed (any mismatch ⇒ stay closed). The Android display screen sets `FLAG_SECURE`. Deferred to follow-ups (the #24 scope cut): the printable recovery sheet (RECOVERY §3), randomized full-entry order + per-field show-toggle (RECOVERY §11), and the biometric lock on the entry screen.

**D8 — Test-vector ratification (the all-zeros mnemonic), host-side.** CRYPTO.md §2's placeholder vector is ratified by this implementation and asserted in `jvmTest` (Bouncy Castle is pure-JVM, so the **entire** vector — `seed`, `prk`, `ed25519_priv`, `x25519_priv`, `ed25519_pub`, `x25519_pub` — is frozen and verified host-side/CI, no device required). The provider's sign→verify round-trip (BC Ed25519) is **also** host-tested in `jvmTest`, and the fail-closed provider semantics (not-provisioned ⇒ `null`; provisioned ⇒ keys) are proven host-side against an in-memory `SecureKeyStorage` double. Only the real `EncryptedSharedPreferences`/StrongBox persistence is exercised on a device (`androidInstrumentedTest`), and nothing in #24's correctness gate depends on that device run. The ratified hex is written back into CRYPTO.md §2 in the same PR.
...
- **Doc corrections in-PR:** RECOVERY.md §2 params and CRYPTO.md §2 ratified vector + §3 library rows. These are consistency fixes that flow from D2/D3/D8, not a re-scope.
