...
- **Config-change retention (Codex F6).** `PairingFlowScreen` starts the attempt in `LaunchedEffect` and cancels it in `DisposableEffect.onDispose`, so an Activity recreation (e.g. rotation) burns the in-flight attempt. Acceptable for this slice (attestation is disabled, so no attempt can complete yet), but the follow-up is to retain the controller across config changes (a `ViewModel`/retained holder) so only an explicit Back/Cancel tears the attempt down.
