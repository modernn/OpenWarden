...
> **Amended by ADR-029 (Proposed, 2026-06-18):** for the committed Samsung/OnePlus **Tier-2** targets (ADR-026), **D2 check 1** widens to accept an **allow-listed OEM attestation root** (Google + Samsung Knox + OnePlus, not only Google) and **D2 check 3** widens to accept **TEE-level** attestation (`TRUSTED_ENVIRONMENT`, not only `STRONGBOX`); **D7**'s Pixel-only scope widens to include them. **Refuse-closed on any *unknown* root or `SOFTWARE` level; VERIFIED boot, locked bootloader, the model allow-list, and the four-key SAS (D2a) all stay mandatory; the downgrade is disclosed (ADR-026 D3).** Tier 1 (Pixel) is unchanged. See ADR-029.
