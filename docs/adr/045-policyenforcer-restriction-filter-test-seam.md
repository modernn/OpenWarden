...
- **A. Injectable `restrictionFilter` seam (chosen).** Add a constructor seam `restrictionFilter: (String) -> Boolean = { true }` to `PolicyEnforcer`, applied to `requiredRestrictions`. Production constructs `PolicyEnforcer(context)` → the default identity filter → the **complete** baseline, byte-for-byte unchanged. Instrumented tests construct `PolicyEnforcer(context, restrictionFilter = { it != UserManager.DISALLOW_DEBUGGING_FEATURES })` so they can run `applyDayOneRestrictions()` on a real provisioned device, verify the rest of the baseline by readback, and keep adb alive. This mirrors the existing `lock` seam exactly (production default + test injection); the variation is **test-wiring**, not a runtime build flag.
...
**D2 — The seam is test-wiring; release is never relaxed at runtime.** No production code passes a non-identity filter. There is no `BuildConfig`/runtime branch on the restriction set. The only callers that narrow the set are instrumented tests, which inject `{ it != UserManager.DISALLOW_DEBUGGING_FEATURES }` to keep adb alive while asserting the rest of the enforced baseline (criterion 2 automation, the consumer in #131). This preserves ADR-020 D1 for release while making the enforcement path testable.
...
**Good:**
- Exit criterion 2 (enforced baseline intact) becomes automatable on a provisioned device with adb alive, closing the gap `E2E_EXIT_CRITERIA.md` documents — without weakening release.
