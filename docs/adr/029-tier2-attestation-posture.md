...
**D3 — The downgrade is DISCLOSED, and that disclosure is release-gating (binds ADR-026 D3 / ADR-023 D5).** Accepting OEM-root + TEE-level attestation is a real reduction in the anti-device-swap (H3 / DEFENSES #14) hardware-genuineness proof versus Pixel StrongBox + Google-root. The pairing/onboarding flow **must** show the parent, before pinning: *"this device's hardware identity is verified at TEE level via <OEM>'s attestation root, not Google's hardware root — slightly weaker physical-tamper resistance than a Pixel"* — and require acknowledgement. Silent acceptance would be the over-promise the non-negotiables forbid.

**D4 — Accepted residual (the security-meaningful consequence the maintainer is signing off).** TEE attestation **still proves genuine OEM hardware** (the OEM's attestation key signs the chain) and **still binds the identity key to the device**. What it loses versus StrongBox is **physical-extraction / side-channel resistance** of the key store — a discrete secure element (Titan M2, Knox Vault) survives EM-glitching and probing better than TEE-on-SoC. For OpenWarden's **declared threat model** (ATTACKS §1: a kid with `adb` + YouTube, **no JTAG, no oscilloscope, no nation-state**), TEE is acceptable: a kid cannot extract a TEE-bound key. This residual is **accepted, scoped to that threat model, and disclosed (D3)** — it is **not** a fail-open (the key remains hardware-bound, the chain still validates to a known allow-listed root, and VERIFIED-boot + the four-key SAS still hold). If the threat model ever expands to physical attackers, this is the line that must be revisited.
...
**D6 — `oem_roots.json` scope + update path.** Ships in `:shared:crypto` with **only** the committed set's roots (Google + Samsung Knox + OnePlus). Verification is **parent-side**; adding an OEM root is a **parent-app update**, and any change that widens the trusted-root set is recovery-phrase-gated. An unknown root is **never** auto-trusted — it surfaces `unverified_attestation` and refuses to pin at the committed tier (ANDROID_COMPAT §4).
