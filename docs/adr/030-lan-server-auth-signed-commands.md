...
- **State:** `/lock` then `GET /state` ⇒ `is_locked == true`; `/unlock` ⇒ `false`.
- **Endpoint auth:** unsigned/garbage body to `/lock` and `/unlock` ⇒ `400`, lock flag unchanged.
