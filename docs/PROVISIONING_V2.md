...
| **S8 PAIRED_WITH_PARENT** | S7 + parent app running, both devices on shared LAN or BLE | Child generates a StrongBox EC P-256 device-binding key `K_bind` (nonce-challenge attestation) that signs over its TEE-resident Ed25519 + X25519 identity keys ([ADR-032](adr/032-child-identity-hardware-binding-strongbox-p256.md); StrongBox can't hold Curve25519); response carries `(child_pub_ed25519, child_pub_x25519, attestation_cert_chain for K_bind, child_binding_sig, pairing_nonce)`; parent verifies the K_bind chain rooted in Google + the binding sig, pins child pubkeys + pushes parent pubkey + initial signed policy bundle *(QR direction predates ADR-025 — separate reconciliation)* | Phrase-gated rotation via §15 in `DEFENSES.md` | H3 pubkey substitution if attestation cert chain not validated parent-side; QR replay if nonce not single-use |
