...
Lives at [`docs/test-vectors/event-log/sealed_envelope.json`](../test-vectors/event-log/sealed_envelope.json), generated by the committed `gen_sealed_envelope.py` (PyNaCl / reference libsodium). Fields: `plaintext_utf8` + `plaintext_hex`, `recipient_x25519_pub_hex` / `recipient_x25519_priv_hex`, a `child_x25519_*_hex` keypair that MUST fail to open, `ephemeral_x25519_priv_hex` (fixed, so `sealed_hex` is deterministic), `sealed_hex`, `sealed_len`, `seal_overhead_bytes: 48`, `open_recovers: true`, `child_open_fails: true`.

`crypto_box_seal` is non-deterministic in production (random ephemeral), so the canonical KAT pins the **open** direction: by fixing the ephemeral key the generator emits stable `sealed_hex`, and the implementation MUST open it to `plaintext_utf8` under the recipient keypair (byte-for-byte libsodium interop) while the child keypair MUST fail (ADR-044 D3/D5). Ratified + asserted in `SealedBoxTest`.
