...
/**
 * RFC 8785 (JCS) canonicalization — the single signing input for bundles and
 * event entries (ADR-015: sign the JCS of the object with its `sig` field removed).
 *
 ...
 *   (PROTOCOL.md §3.1 rule: integers only, no floats), so ECMAScript double
 *   formatting never applies; integer fields are additionally bounded to
 *   0..2^53-1 at validation time via [requireJcsSafe] (ADR-017)
 * - minimal RFC 8259 string escaping; output is UTF-8 when encoded to bytes
 */
object Canonical {
    /** 2^53 − 1: the largest integer JCS number formatting round-trips exactly (ADR-017). */
